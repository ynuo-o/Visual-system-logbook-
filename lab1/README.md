...
[rows, cols] = size(In)
 ```
This ensures the rotated image `Out` is created with the same spatial resolution as the original image:
```matlab
Out = zeros(rows, cols);
 ```
Pixels that do not receive valid mappings are therefore displayed as black (`0`).
